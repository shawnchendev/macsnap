...
/// Resident menu-bar frontend for macsnap. Never captures itself — each menu
/// action launches the installed `macsnap` binary, which keeps the existing
...
final class MenuBarDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        button.toolTip = "macsnap \(MacsnapApp.version) — screenshot capture"
        showWelcomeIfNeeded()
        startHotkeys()
    }
        ...
        for action in HotkeyManager.Action.allCases {
            if let binding = saved[action.rawValue] {
                if !hotkeys.register(action: action, binding: binding) {
                    NSLog("macsnap-menubar: could not grab saved hotkey %@ for %@", binding.description, action.rawValue)
                }
            }
        }
    }
    ...
    private func applyHotkeys(_ config: AppConfig) -> [String] {
        ...
        for action in HotkeyManager.Action.allCases {
            if let binding = config.hotkeys[action.rawValue] {
                if hotkeys.register(action: action, binding: binding) {
                    NSLog("macsnap-menubar: hotkey %@ -> %@", action.rawValue, binding.description)
                } else {
    ...
    private func showWelcomeIfNeeded() {
        ...
        let alert = NSAlert()
        alert.messageText = "macsnap is running in your menu bar"
    ...
    private func makeStatusItem() -> NSStatusBarButton? {
        ...
        if let icon = NSImage(systemSymbolName: "camera.viewfinder", accessibilityDescription: "macsnap") {
        ...
        menu.addItem(withTitle: "Settings…", action: #selector(openSettings(_:)), keyEquivalent: ",")
            .target = self
        menu.addItem(.separator())
        menu.addItem(withTitle: "Quit macsnap menu bar", action: #selector(quit(_:)), keyEquivalent: "q")
            .target = self
        ...
        statusMenu = menu
        statusItem = item
        NSLog("macsnap-menubar: status item installed with %d menu items", menu.items.count)
        refreshLoginItem()
        return button
    }
    ...
    @objc private func statusItemClicked(_ sender: Any?) {
        ...
        NSLog("macsnap-menubar: status item clicked, popping menu")
    ...
    func menuWillOpen(_ menu: NSMenu) {
        NSLog("macsnap-menubar: menu will open")
        refreshLoginItem()
    ...
    private func runCapture(arguments: [String]) {
        ...
        if now.timeIntervalSince(lastCaptureLaunch) < 1.0 {
            NSLog("macsnap-menubar: ignoring duplicate capture launch")
            return
        }
        ...
        guard let bin = MenuBarSupport.macsnapBinaryURL(ownExecutableURL: ownExec, pathEnv: pathEnv) else {
            let alert = NSAlert()
            alert.messageText = "macsnap binary not found"
            alert.informativeText = "Install it with `make install` so it sits next to the menu-bar app."
            alert.alertStyle = .warning
            alert.runModal()
            return
        }
        NSLog("macsnap-menubar: launching %@ %@", bin.path, arguments.joined(separator: " "))
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = "Could not launch macsnap"
    ...
    private func showCaptureFailure(stderr: String) {
        ...
        let detail = stderr.isEmpty
            ? "macsnap exited without capturing. If this was launched from the app bundle, grant Screen Recording to “macsnap” in System Settings first."
            : stderr
        alert.informativeText = detail + "\n\nDetails: ~/.local/state/macsnap/menubar.log"
        alert.alertStyle = .warning
