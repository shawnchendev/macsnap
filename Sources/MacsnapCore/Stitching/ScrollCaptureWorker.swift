...
/// Background capture loop for scrolling-region capture, mirroring omasnap's
/// `ScrollCapturePanel` worker threads.
///
...
public final class ScrollCaptureWorker: @unchecked Sendable {
    ...
    // Longest edge most image software reliably opens (omasnap advisory).
    ...
    public func start() {
        ...
        let q = DispatchQueue(label: "com.macsnap.scrollcapture", qos: .userInitiated)
        self.queue = q
    ...
    /// keeping every verified band, like omasnap's `continueCapture`.
    public func resume() {
        lock.lock()
        stalled = false
        ended = false
    ...
    private func checkBudget(crossPx: Int, axisBasePx: Int) -> Bool {
        ...
        if axisNow > Self.kWidelyOpenableEdge {
            // Advisory only (omasnap keeps capturing); surfaced via `full` hint.
            lock.lock(); full = true; lock.unlock()
        }
        return false
    }

    // MARK: - Manual loop: classify every poll, like omasnap's captureLoop
    ...
    // MARK: - Auto loop: inject → settle → feed, like omasnap's autoCaptureLoop
    ...
    private func autoLoop(firstExtentPx: Int, crossPx: Int) {
        ...
        // generously at any scroll speed (omasnap kTargetStepFraction).
        ...
        while !isStopped() {
            ...
            // Wait for motion to start (omasnap kMotionWaitMs).
            ...
            // Settle: two consecutive identical grabs (omasnap settle), cap 2s.
            ...
            case .forward where motion.delta > 2:
                ...
                // construction; no notch fitting needed (omasnap fits detents
                // because one notch moves a different distance per app).
            case .stationary:
                stationaryStreak += 1
    ...
    private func stallUnavailable() {
        ...
        // frames and fall back to manual on the same session (omasnap does).
        lock.lock()
