...
/// Pure helpers behind the resident menu-bar app (see the `MacsnapMenuBar`
/// target). The menu bar itself never captures — it just launches the
/// installed `macsnap` binary, so Screen Recording permission stays with the
/// binary the user already approved.
public enum MenuBarSupport {
    public static let launchAgentLabel = "com.macsnap.menubar"

    /// Locates the `macsnap` capture binary: next to our own executable first
    /// (both are installed into the same `bin` dir by `make install`), then
    /// each entry of `PATH`.
    public static func macsnapBinaryURL(ownExecutableURL: URL?, pathEnv: String) -> URL? {
        let fm = FileManager.default
        if let own = ownExecutableURL {
            let sibling = own.deletingLastPathComponent().appendingPathComponent("macsnap")
            if fm.isExecutableFile(atPath: sibling.path) {
                return sibling
            }
        }
        for dir in pathEnv.split(separator: ":").map(String.init) {
            let candidate = URL(fileURLWithPath: dir).appendingPathComponent("macsnap")
    ...
    public static func menubarLockURL() -> URL {
        URL(fileURLWithPath: "/tmp/macsnap-\(getuid())/menubar.lock")
    }

    public static func diagLogURL(home: URL) -> URL {
        home.appendingPathComponent(".local/state/macsnap/menubar.log")
    }
    ...
    public static func appStateDir(home: URL) -> URL {
        home.appendingPathComponent(".local/state/macsnap")
    }
