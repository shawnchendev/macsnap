...
/// scrollable and the overlay never appears in its own screenshots). This
/// small panel is the only chrome, and it
/// is always placed fully outside the capture region so it is never stitched
/// into the result. During capture the keyboard belongs to the page, so
/// the on-screen buttons are the controls; no keys are promised.
