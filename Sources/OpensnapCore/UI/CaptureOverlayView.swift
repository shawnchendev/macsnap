...
public final class CaptureOverlayView: NSView, NSTextFieldDelegate {
    ...
    // Scrolling capture state (flow: drag region → pick
    ...
    private func drawSelectPhase(in context: CGContext) {
        ...
        // known window gets a faint outline and the hovered one is
    ...
    public override func mouseDown(with event: NSEvent) {
        ...
        if phase == .select {
            ...
            // else starts a fresh region (chrome clicks never restart).
            if scrollPhase == .modeChoice {
            ...
            if captureKind == .window {
                // Auto-capture the window under the press
    ...
    public override func keyDown(with event: NSEvent) {
        ...
        // Arrow keys in window mode: Cmd+Arrows move
    ...
    /// × vertical/horizontal. The region is shared
    ...
    public func enterScrollModeChoice(region: CGRect) {
        ...
        // Captures smaller than this are not offered.
        guard clean.width >= 32 && clean.height >= 32 else { return }
        selection = clean
        lastDrawnRegion = clean
        committedScreenSelection = clean
        scrollPhase = .modeChoice
        scrollStatusText = "Choose how to capture · Manual: you scroll · Auto: opensnap scrolls"
        scrollStatusWarning = false
        needsDisplay = true
    }

    /// Starts the live capture: hides the overlay (the page underneath must
    /// stay scrollable and the overlay must not photograph itself) and shows
    /// the floating HUD panel outside the region with Done/Back/Cancel.
    ...
    /// Back to the mode row with the region intact.
    ...
    public func cancelScrollCapture() {
        ...
        // Mode choice → back to plain selecting (region kept).
        scrollPhase = .idle
        scrollModeButtons = []
        scrollCancelButtonRect = .zero
        needsDisplay = true
    ...
    private func activateCaptureKind(_ kind: CaptureKind) {
        ...
        // between the two keeps it. Window and Fullscreen pick an
    ...
    public func finish(outputMode: QuickOutputMode) {
        ...
        // shelf reflects what was actually captured.
