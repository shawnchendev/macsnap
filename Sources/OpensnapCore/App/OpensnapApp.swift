...
public final class OpensnapApp: NSObject, NSApplicationDelegate {
    ...
    private func startApp(args: [String]) async {
        ...
        while i < args.count {
            ...
            switch arg {
            case "--version", "-v":
                print("opensnap \(Self.version) (native macOS screenshot and annotation editor)")
                exit(0)
            case "--help", "-h":
                printHelp()
                exit(0)
        ...
        if !ScreenCaptureEngine.hasScreenRecordingPermission() {
            fputs("[opensnap] Screen Recording permission is required.\n", stderr)
            fputs("[opensnap] \(MenuBarSupport.permissionPromptMarker) — no further dialog needed.\n", stderr)
        ...
        guard let capture = await ScreenCaptureEngine.captureCurrentScreen(includeWindows: !instantFullscreenOutput) else {
            ...
            fputs("If Screen Recording already looks enabled for opensnap, remove it with minus and re-add it (each rebuild changes an ad-hoc signature).\n", stderr)
    ...
    private func printHelp() {
        print("""
        Usage: opensnap [options] [image_path]

        Native macOS screenshot and annotation editor.

        Capture Modes:
          opensnap                     Freeform region selection (default)
          opensnap --capture-region    Region capture mode
          opensnap --capture-window    Window capture mode (hover & arrow navigation)
          opensnap --capture-fullscreen Fullscreen capture
          opensnap --scroll            Scrolling region capture
          opensnap region | windows | fullscreen | smart
        ...
        Image Editing:
          opensnap <path>              Open existing image in annotation editor
          opensnap --file <path>       Open existing image in annotation editor
          opensnap --clipboard         Open image currently on clipboard
          opensnap --pin <path>        Open image in floating pinned mode

        Single Instance:
          Running opensnap while an overlay is active dismisses the overlay,
