...
final class WindowAndScrollTests: XCTestCase {
    ...
    func testOpensnapBinaryPrefersSibling() {
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        try! FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
        let sibling = dir.appendingPathComponent("opensnap")
        ...
        let found = MenuBarSupport.opensnapBinaryURL(
            ownExecutableURL: dir.appendingPathComponent("opensnap-menubar"),
            pathEnv: "/nonexistent"
        )
        XCTAssertEqual(found?.path, sibling.path)
    }

    func testOpensnapBinaryFallsBackToPATH() {
        let found = MenuBarSupport.opensnapBinaryURL(
            ownExecutableURL: URL(fileURLWithPath: "/nonexistent/opensnap-menubar"),
            pathEnv: "/nonexistent"
        )
        XCTAssertNil(found)
    }
    ...
    func testLaunchAgentPlistRoundTrip() {
        ...
        try! MenuBarSupport.setLoginItemEnabled(true, menubarProgram: "/tmp/fake/opensnap-menubar", home: home)
        ...
        XCTAssertTrue(plist.contains("com.opensnap.menubar"))
        XCTAssertTrue(plist.contains("/tmp/fake/opensnap-menubar"))
        try! MenuBarSupport.setLoginItemEnabled(false, menubarProgram: "", home: home)
        XCTAssertFalse(MenuBarSupport.isLoginItemEnabled(home: home))
    }

    func testShouldOfferSettingsButton() {
        XCTAssertTrue(MenuBarSupport.shouldOfferSettingsButton(stderr: "[opensnap] Screen Recording permission is required."))
    ...
    func testShouldSuppressCaptureFailureDialog() {
        ...
        XCTAssertTrue(MenuBarSupport.shouldSuppressCaptureFailureDialog(
            stderr: "[opensnap] Screen Recording permission is required.\n[opensnap] \(MenuBarSupport.permissionPromptMarker) — no further dialog needed.",
    ...
    func testSaveToRecentShelvesCapture() {
        ...
        defer {
            unsetenv("OPENSNAP_RECENT_DIR")
            try? FileManager.default.removeItem(at: recent)
        }
        setenv("OPENSNAP_RECENT_DIR", recent.path, 1)
